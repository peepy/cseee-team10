...
void loop(){
  ...
  // for more details
  tb.sendTelemetryFloat("temperature", (float)currentTemperature / 100);
  tb.sendTelemetryFloat("pH", (float)currentPH / 100);
...
//Random Data
int getCurrentTemperature(){
  int temperature = ((float)rand()/(float)(RAND_MAX)) * 60 * 100; // Multiple by 100 for fixed point noation
  return temperature;
}

int getCurrentPH(){
  int ph = ((float)rand()/(float)(RAND_MAX)) * 14 * 100; // Multiply by 100 for fixed point notation
  return ph;
}
